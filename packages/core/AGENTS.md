...
```ts
import {
  state, compute, watch, batch,       // reactive
  html, css,                           // template + styles
...
component('p-form', () => {
  const name = state('');
  const submit = (e) => { e.preventDefault(); console.log(name()); };
...
match(() => status(), { loading: () => html`...`, error: () => html`...`, success: () => html`...` })
```
...
- Biome formatter (2-space, single quotes, trailing commas)
