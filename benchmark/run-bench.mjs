...
console.log(
  `${pad('Operation', 28)} ${pad('Median', 10)} ${pad('Mean', 10)} ${pad('σ', 10)} ${pad('Min', 10)} ${pad('Max', 10)}`,
);
...
for (const r of results) {
  if (r.error || r.median == null) {
    ...
    console.log(
      `${pad(r.name, 28)} ${pad(`${r.median.toFixed(1)}ms`, 10)} ${pad(`${r.mean.toFixed(1)}ms`, 10)} ${pad(`${r.stddev.toFixed(1)}ms`, 10)} ${pad(`${r.min.toFixed(1)}ms`, 10)} ${pad(`${r.max.toFixed(1)}ms`, 10)}`,
    );
  }
}
