 ...
 * `js/bad-code-sanitization` finding GitHub Advanced Security raises on
 * the string templates below.
 *
 * Every value spliced into the generated JS source goes through
 * {@link jsStringLiteral}, which wraps `JSON.stringify` and additionally
 * escapes U+2028 / U+2029. `JSON.stringify` alone handles quotes,
 * backslashes, newlines, control chars, and lone surrogates — but it
 * leaves U+2028 / U+2029 raw, and those code points are valid string-
 * literal *terminators* under ES2018-and-earlier parsing rules, which is
 * exactly the residual gap `js/bad-code-sanitization` flags. Escaping
 * them makes `jsStringLiteral` a complete sanitizer for the
 * "embed-as-double-quoted-string-literal" context: the string-template
 * positions (`${fp}`, `${abs}`, `${pattern}`, `${filePath}`) all surround
 * its output, so no value can escape the literal regardless of parser
 * version.
 *
 ...
 * named file can only flow into the manifest as a fully-escaped string
 * literal.
 */

/**
 * Encode `value` as a JS double-quoted string literal safe to splice
 * into generated source. `JSON.stringify` + explicit U+2028 / U+2029
 * escaping — the documented remediation for `js/bad-code-sanitization`
 * when embedding a value inside generated code.
 */
...
function entryLiteral(e: LayoutEntry, absPathFor: (filePath: string) => string): string {
  const fp = jsStringLiteral(e.filePath);
  const abs = jsStringLiteral(absPathFor(e.filePath));
  const loaderPart = e.hasLoader ? ', hasLoader: true' : '';
  const dirPart = e.dir !== undefined ? `, dir: ${jsStringLiteral(e.dir)}` : '';
  // codeql[js/bad-code-sanitization]
  return `{ filePath: ${fp}, importFn: () => import(${abs})${loaderPart}${dirPart} }`;
}

/**
 * Codegen for the per-app sibling `.d.ts` next to the emitted `.ts`
 * manifest (ADR 0044). Augments the virtual `'purity:routes'` module
 * with literal tuple types whose `importFn` is `() => Promise<typeof
 * import('<abs>')>` per-route, so apps that import from
 * `'purity:routes'` get the same strong types as apps that import
 * from the on-disk file.
 *
 * Emitted alongside the `.ts` whenever `emitTo` is set. The literal
 * absolute paths come from the same `absPathFor` callback used by
 * `generateRouteManifestSource`.
 */
function entryTypeLiteral(e: LayoutEntry, absPathFor: (filePath: string) => string): string {
  const abs = jsStringLiteral(absPathFor(e.filePath));
  const fp = jsStringLiteral(e.filePath);
  const loaderPart = e.hasLoader ? '; readonly hasLoader: true' : '';
  const dirPart = e.dir !== undefined ? `; readonly dir: ${jsStringLiteral(e.dir)}` : '';
  // codeql[js/bad-code-sanitization]
  return `{ readonly filePath: ${fp}; readonly importFn: () => Promise<typeof import(${abs})>${loaderPart}${dirPart} }`;
}

export function generateRouteManifestTypes(
  manifest: RouteManifest,
  ...
  for (const e of manifest.routes) {
    const abs = jsStringLiteral(absPathFor(e.filePath));
    const pattern = jsStringLiteral(e.pattern);
    const filePath = jsStringLiteral(e.filePath);
    const layouts = e.layouts.map((l) => entryTypeLiteral(l, absPathFor)).join(', ');
  ...
  if (manifest.notFound) {
    lines.push(`  export const notFound: ${entryTypeLiteral(manifest.notFound, absPathFor)};`);
  } else {
    lines.push('  export const notFound: undefined;');
  }
  const chain = manifest.notFoundChain ?? [];
  lines.push('  export const notFoundChain: readonly [');
  for (const e of chain) {
    lines.push(`    ${entryTypeLiteral(e, absPathFor)},`);
  }
  lines.push('  ];');
  lines.push('  export type { LayoutEntry, RouteEntry };');
  lines.push('}');
  lines.push('');
  return lines.join('\n');
}
