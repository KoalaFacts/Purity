...
describe("parser", () => {
  it("parses a simple element", () => {
    const ast = parse(["<div></div>"]) as any;
  ...
  it("parses nested elements", () => {
    const ast = parse(["<div><p></p></div>"]) as any;
    expect(ast.children[0].type).toBe("element");
  ...
  it("parses text content", () => {
    const ast = parse(["<p>Hello world</p>"]) as any;
  ...
  it("parses static attributes", () => {
    const ast = parse(['<div class="box" id="main"></div>']) as any;
  ...
  it("parses @event attribute", () => {
    const ast = parse(["<button @click=", ">Click</button>"]) as any;
    const btn = ast.children[0];
    expect(btn.attributes[0]).toEqual({
      kind: "event",
      name: "click",
      index: 0,
    });
  ...
  it("parses ?bool attribute", () => {
    const ast = parse(["<input ?disabled=", " />"]) as any;
    const input = ast.children[0];
    expect(input.attributes[0]).toEqual({
      kind: "bool",
      name: "disabled",
      index: 0,
    });
  ...
  it("parses .prop attribute", () => {
    const ast = parse(["<input .value=", " />"]) as any;
    const input = ast.children[0];
    expect(input.attributes[0]).toEqual({
      kind: "prop",
      name: "value",
  ...
  it("parses :reactive-prop attribute", () => {
    const ast = parse(["<p-card :title=", "></p-card>"]) as any;
    const el = ast.children[0];
    expect(el.attributes[0]).toEqual({
      kind: "reactive-prop",
      name: "title",
  ...
  it("parses dynamic attribute", () => {
    const ast = parse(["<div class=", "></div>"]) as any;
    const el = ast.children[0];
    expect(el.attributes[0]).toEqual({
      kind: "dynamic",
      name: "class",
  ...
  it("parses void elements", () => {
    const ast = parse(['<br/><input type="text" />']) as any;
  ...
  it("parses multiple expressions", () => {
    ...
    const ast = parse(["<p>", "</p><p>", "</p>"]) as any;
  ...
  it("parses mixed static and expression attributes", () => {
    const ast = parse(['<div class="box" id=', " @click=", "></div>"]) as any;
    const el = ast.children[0];
...
describe("compiled html`` performance", () => {
  ...
  it("renders 1k elements with reactive bindings under 200ms", () => {
    ...
    for (let i = 0; i < 1000; i++) {
      void html`<div class=${() => (count() > 5 ? "active" : "")}>
        <p>${() => count()}</p>
      </div>`;
    }
    const elapsed = performance.now() - start;
