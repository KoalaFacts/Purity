...
let tagCounter = 0;
const tag = (base: string) => `p-${base}-${tagCounter++}`;
...
describe("slots — callback syntax", () => {
  it("consumer fills default slot via callback", () => {
    ...
    container.appendChild(
      html`${Card({ title: "Hi" }, ({ default: fill }: any) => fill(html`<p>Body</p>`))}`,
    );
  ...
  it("consumer fills named slots via callback", () => {
    ...
    const container = document.createElement("div");
    container.appendChild(
      html`${Layout({}, ({ header, default: body, footer }: any) => {
