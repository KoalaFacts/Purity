...
defineExpose({
  run(count: number) { data.value = mkData(count); selectedId.value = 0; },
  add() { data.value = data.value.concat(mkData(1000)); },
  update() {
    const c = data.value.slice();
    for (let i = 0; i < c.length; i += 10) c[i] = { ...c[i], label: `${c[i].label} !!!` };
    data.value = c;
  },
  select(id: number) { selectedId.value = id; },
  swapRows() {
    const d = data.value;
    if (d.length > 998) {
      const c = d.slice();
  ...
  remove(id: number) { data.value = data.value.filter(x => x.id !== id); },
  clear() { data.value = []; selectedId.value = 0; },
