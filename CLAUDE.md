...
- ADR [0006](./docs/decisions/0006-streaming-suspense.md) (Proposed) sketches `suspense(view, fallback)` + `renderToStream` for progressive rendering. Phases 1, 2, 3, 5 are shipped: `renderToStream` returns a `ReadableStream<Uint8Array>` that flushes the shell with each boundary's fallback, then drains streamed boundary chunks (`<template id="purity-s-N">…</template><script>__purity_swap(N)</script>`) in declaration order. The `__purity_swap` helper is inlined exactly once after the shell. Phase 4 (adapter examples) and Phase 6 second-half (per-boundary resource cache emit) remain.
